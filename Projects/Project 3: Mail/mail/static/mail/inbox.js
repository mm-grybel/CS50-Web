  ...
  .then(emails => {
    ...
    emails.forEach(email => {
      ...
      if (email.body.length <= 99) {
        div.innerHTML = `
          <div class='card'>
            <div class='card-header ${is_read}'><strong>${email.subject}</strong></div>
            <div class='card-body ${is_read}' id='item-${email.id}'>
              ...
              <p class="card-text">${email.body.slice(0, 99)}</p>
              <a href="#" class="btn btn-primary"><i class="fas fa-book-reader"></i> Read</a>
            </div>
          </div>
        `;
      } else {
        div.innerHTML = `
          <div class='card'>
            ...
            <div class='card-body ${is_read}' id='item-${email.id}'>
              ...
              <p class="card-text">${email.body.slice(0, 99)} <a href='#'>(more...)</a></p>
              <a href="#" class="btn btn-primary"><i class="fas fa-book-reader"></i> Read</a>
            </div>
          </div>
        `;
...
function view_email(email_id, mailbox) {
  ...
  .then(email => {
    ...
    div.innerHTML = `
      <div class='card'>
        <div class='card-header'><strong>${email.subject}</strong></div>
        <div class='card-body' id='item-${email.id}'>
          <p class='card-title'><strong>From:</strong> ${email.sender} &nbsp; |  &nbsp; <strong>To: </strong> ${email.recipients} &nbsp; |  &nbsp; <strong>Date:</strong> ${email.timestamp} <br></p>
          <p class="card-text">
